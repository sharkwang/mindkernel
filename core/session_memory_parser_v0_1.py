#!/usr/bin/env python3
"""Core parser: OpenClaw session JSONL -> memory-event candidates (v0.1)."""
...
def make_event_id(event_type: str, source_ref: str, event_key: str | None = None) -> str:
    key = f"{event_type}|{source_ref}|{event_key or ''}"
...
def classify_user_event(text: str) -> tuple[str, str, float, str] | None:
    ...
    if RE_DIRECTIVE.search(t):
        return ("workflow_directive", f"用户发出执行指令：{compact_text(t)}", 0.98, "high")

    if RE_REQUEST.search(t) or "?" in t or "？" in t:
        return ("user_request", f"用户请求：{compact_text(t)}", 0.97, "medium")
...
def classify_assistant_event(text: str) -> tuple[str, str, float, str] | None:
    ...
    if RE_MILESTONE_CLAIM.search(t_nocode):
        ...
        return ("milestone_completed", f"{label} 完成：{compact_text(t_nocode)}", 0.94, "high")

    if RE_FORMAT_DISCOVERY.search(t_nocode):
        return ("format_discovery", f"结构发现：{compact_text(t_nocode)}", 0.90, "medium")
...
def build_event(
    *,
    ...
    return {
        "id": make_event_id(event_type, source_ref, event_key=event_key),
        "kind": kind,
        "event_type": event_type,
        "content": content,
        "source": {"source_type": "session", "source_ref": source_ref},
        ...
        "status": "candidate",
        "observed_at": ts,
        "migration_meta": {"session_line": line_no, "role": role, "msg_id": msg_id},
    }


def event_to_memory_object(event: dict, review_due_days: int, next_action_days: int, status: str) -> dict:
    ...
    for idx, item in enumerate(content_items):
        ...
        events.append(
            build_event(
                ...
                kind="event",
                role="assistant",
                event_key=f"tool:{idx}:{name}:{hashlib.sha1(args_txt.encode('utf-8')).hexdigest()[:8]}",
            )
        )
    return events


def parse_session(session_file: Path, include_tool_calls: bool, max_events: int) -> dict:
    ...
    with session_file.open() as f:
        for line_no, line in enumerate(f, start=1):
            ...
            text = "\n".join(c.get("text", "") for c in content_items if c.get("type") == "text" and c.get("text")).strip()
    ...
    return {
        ...
        "summary": {"count": len(events), "by_type": dict(by_type)},
